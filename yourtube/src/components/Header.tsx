...
const Header = ({ onMenuClick }: HeaderProps) => {
  ...
  useEffect(() => {
    if (user?.themePreference) {
      ...
      if (user.themePreference === "light") root.classList.remove("dark");
      else if (user.themePreference === "dark") root.classList.add("dark");
    }
  }, [user]);
  ...
  const handleThemeChange = async (selectedTheme: "light" | "dark" | "auto") => {
    ...
    try {
      ...
      if (response.status === 200) {
        const root = window.document.documentElement;
        if (selectedTheme === "light") root.classList.remove("dark");
        else if (selectedTheme === "dark") root.classList.add("dark");
  ...
  return (
    <>
      <header className="flex items-center justify-between px-4 lg:px-6 xl:px-8 py-2 bg-white dark:bg-[#0f0f0f] text-black dark:text-white border-b dark:border-zinc-800 sticky top-0 z-50 h-[57px] select-none transition-colors duration-150">
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          ...
          <Link href="/" className="flex items-center gap-1">
            ...
            <span className="text-xl font-medium tracking-tight text-black dark:text-white hidden sm:inline">YourTube</span>
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          <Button
            ...
            variant="ghost"
            size="icon"
            className="md:hidden dark:text-white dark:hover:bg-zinc-800"
                ...
                <DropdownMenuContent className="w-56 bg-white dark:bg-zinc-950 text-black dark:text-white border dark:border-zinc-800" align="end" forceMount>
                  {user?.channelname ? (
                    ...
                    <div className="px-2 py-1.5">
                      <Button variant="secondary" size="sm" className="w-full" onClick={() => setisdialogeopen(true)}>Create Channel</Button>
                    </div>
                  )}
                  <DropdownMenuItem asChild className="dark:focus:bg-zinc-800 cursor-pointer"><Link href="/history">History</Link></DropdownMenuItem>
                  <DropdownMenuItem asChild className="dark:focus:bg-zinc-800 cursor-pointer"><Link href="/liked">Liked videos</Link></DropdownMenuItem>
                  <DropdownMenuItem asChild className="dark:focus:bg-zinc-800 cursor-pointer"><Link href="/watch-later">Watch later</Link></DropdownMenuItem>
                  <DropdownMenuSeparator className="dark:bg-zinc-800" />
                  <div className="px-2 py-1 text-[11px] font-bold text-zinc-400 dark:text-zinc-500 tracking-wider uppercase">Appearance Theme</div>
                  <DropdownMenuItem onClick={() => handleThemeChange("light")} className="dark:focus:bg-zinc-800 cursor-pointer flex items-center gap-2">
                    <Sun className="w-4 h-4" /> Light Mode
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleThemeChange("dark")} className="dark:focus:bg-zinc-800 cursor-pointer flex items-center gap-2">
                    <Moon className="w-4 h-4" /> Dark Mode
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleThemeChange("auto")} className="dark:focus:bg-zinc-800 cursor-pointer flex items-center gap-2">
                    <Clock className="w-4 h-4" /> Auto (Time Based)
                  </DropdownMenuItem>
                  <DropdownMenuSeparator className="dark:bg-zinc-800" />
